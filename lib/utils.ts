 ...
 */
export function replaceCitationsWithLinks(markdownText: string, basePdfUrl: string): string {
  if (!markdownText || !basePdfUrl) return markdownText || ""; // Handle null/undefined/empty input

  return markdownText.replace(/\[(\d+(?:-\d+)?(?:,\s*\d+)*)\]/g, (match, citation) => {
    ...
    if (citation.includes('-')) {
      ...
      const links = Array.from({ length: end - start + 1 }, (_, i) => `[${start + i}](${basePdfUrl}#page=${start + i})`);
      return links.join(' ');
    }
